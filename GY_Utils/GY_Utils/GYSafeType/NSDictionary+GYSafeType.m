...
#import "NSDictionary+GYSafeType.h"
...
+ (void)swapMethod {
    ...
    SEL alterSel = @selector(gySafeDictionaryWithObjects:forKeys:count:);
...
+ (id)gySafeDictionaryWithObjects:(const id [])objects forKeys:(const id<NSCopying> [])keys count:(NSUInteger)cnt{
    ...
    return [self gySafeDictionaryWithObjects:safeObjects forKeys:safeKeys count:j];
}

- (instancetype)gySafeInitWithObjects:(const id [])objects forKeys:(const id <NSCopying> [])keys count:(NSUInteger)cnt {
    ...
    return [self gySafeInitWithObjects:safeObjs forKeys:safeKeys count:j];
}
...
@implementation NSMutableDictionary (GYSafeType)
...
+ (void)swapMethod {
    ...
    Method alterMethod = class_getInstanceMethod(class, @selector(gySafeSetObject:forKey:));
    method_exchangeImplementations(originMethod, alterMethod);
    
    Method originKeyedSubscriptMethod = class_getInstanceMethod(class, @selector(setObject:forKeyedSubscript:));
    Method alterKeyedSubscriptMethod = class_getInstanceMethod(class, @selector(gySafeSetObject:forKeyedSubscript:));
    method_exchangeImplementations(originKeyedSubscriptMethod, alterKeyedSubscriptMethod);
}

- (void)gySafeSetObject:(id)anObject forKey:(id <NSCopying>)aKey {
    
    if (!aKey || !anObject) {
        return;
    }
    [self gySafeSetObject:anObject forKey:aKey];
}

- (void)gySafeSetObject:(id)obj forKeyedSubscript:(id <NSCopying>)key {
    
    if (!key || !obj) {
        return;
    }
    [self gySafeSetObject:obj forKeyedSubscript:key];
}
